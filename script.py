...
COMMENDATION_EXAMPLES = ['Молодец!', 'Отлично!', 'Хорошо!',
...
def get_schoolkid(first_last_name):
    try:
        schoolkid = models.Schoolkid.objects.get(
            full_name__contains=first_last_name)
        return schoolkid
...
def fix_marks(first_last_name):
    ...
    if schoolkid is None:
        return
    for entry in models.Mark.objects.filter(schoolkid=schoolkid,
                                            points__in=[2, 3]):
        entry.points = 5
        entry.save()
...
def remove_chastisements(first_last_name):
    ...
    if schoolkid is None:
        return
    deleted_chastisements_number = \
        models.Chastisement.objects.filter(schoolkid=schoolkid).delete()[0]
...
def remove_commendations(first_last_name):
    ...
    if schoolkid is None:
        return
    deleted_commendations_number = \
        models.Commendation.objects.filter(schoolkid=schoolkid).delete()[0]
...
def create_commendation(first_last_name, lesson):
    ...
    if schoolkid is None:
        return
    last_lesson = models.Lesson.objects.filter(
    ...
    is_commendation_exists = models.Commendation.objects.filter(
    ...
    if not is_commendation_exists:
        ...
        print(
            f'Похвала на урок {last_lesson.subject} от {last_lesson.date} уже '
            f'существует. Запусти функцию еще раз')
